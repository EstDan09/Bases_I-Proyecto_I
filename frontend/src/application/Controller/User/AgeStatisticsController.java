...
public class AgeStatisticsController implements Initializable {
	...
	private void loadChart(ArrayList<PieChart.Data> arrData) {
		...
		chart.setTitle("Age Stadistics by Sport");
...
//		loadChart(ConnectDB.getGenderStatistic("*"));
		...
		ArrayList<PieChart.Data> dummy = new ArrayList<>();
		dummy.add(new PieChart.Data("18-29 20%", 20));
		dummy.add(new PieChart.Data("30-45 40%", 40));
		dummy.add(new PieChart.Data("46-60 25%", 25));
		dummy.add(new PieChart.Data("61-80 15%", 15));
		loadChart(dummy);
